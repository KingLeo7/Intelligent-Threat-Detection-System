...
class Config:
    ...
    def load_config(self):
        ...
        defaults = {
            ...
            "safe_scans_notification": 5  # Show notification after this many safe scans
        }
    ...
    def save_config(self):
        ...
        config_dict = {
            ...
            "safe_scans_notification": self.safe_scans_notification
        }
        try:
...
class FileMonitor:
    ...
    def __init__(self, config):
        ...
        self.callbacks = {
            ...
            'on_alert': None,
            'on_update': None,
            'on_safe_status': None  # New callback for safe status
        }
        self.safe_scan_count = 0  # Track consecutive safe scans
    ...
    def start_monitoring(self):
        ...
        if not self.monitoring:
            ...
            self.safe_scan_count = 0  # Reset counter
    ...
    def stop_monitoring(self):
        ...
        self.monitoring = False
        self.safe_scan_count = 0  # Reset counter when stopped
    ...
    def _monitor_loop(self):
        ...
        while self.monitoring:
            ...
            if len(modified) >= self.config.threshold:
                ...
                # Reset safe scan counter on threat
                self.safe_scan_count = 0
            else:
                # No threats detected - increment safe scan counter
                self.safe_scan_count += 1
                
                # Check if we should show safe status notification
                if self.safe_scan_count == self.config.safe_scans_notification:
                    if self.callbacks['on_safe_status']:
                        safe_msg = (
                            f"✅ ALL CLEAR - NO THREATS DETECTED\n\n"
                            f"Scans Completed: {self.safe_scan_count}\n"
                            f"Files Monitored: {len(current)}\n"
                            f"New Files: {len(new_files)}\n"
                            f"Modified Files: {len(modified)}\n"
                            f"Status: System is secure\n"
                            f"Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
                        )
                        self.callbacks['on_safe_status'](safe_msg)
...
class ThreatDetectionApp:
    ...
    def __init__(self, config, auth_manager):
        ...
        self.monitor.callbacks['on_safe_status'] = self.on_safe_status_callback  # New callback
    ...
    def _create_widgets(self):
        ...
        self.stats_label = tk.Label(
            top_frame,
            text="📊 Files: 0 | Modified: 0 | New: 0 | Safe Scans: 0"
        )
    ...
    def on_safe_status_callback(self, message):
        """Callback when safe status is reached"""
        self.root.after(0, self.log_message, f"✅ System Status: No threats detected after {self.monitor.safe_scan_count} scans")
        self.root.after(0, messagebox.showinfo, "System Status - All Clear", message)
    ...
    def on_update_callback(self, current, new_files, modified, deleted):
        ...
        status_msg = (
            ...
            f"New: {len(new_files)} | "
            f"Safe Scans: {self.monitor.safe_scan_count}"
        )
        if deleted:
...
class SettingsWindow:
    ...
    def _create_widgets(self):
        ...
        # Safe scans notification
        tk.Label(self.window, text="Safe Scans Before Notification:").pack(pady=5)
        self.safe_scans_var = tk.IntVar(value=self.config.safe_scans_notification)
        tk.Spinbox(
            self.window,
            from_=1,
            to=50,
            textvariable=self.safe_scans_var,
            width=10
        ).pack()
    ...
    def save_settings(self):
        ...
        self.config.safe_scans_notification = self.safe_scans_var.get()
